...
import ReviewCart from '@/app/components/review/ReviewCart'
...
const ProductReviewsClient: FC<ProductReviewsClientProps> = ({
	reviews,
	product,
	totalPages
}) => {
	...
	return (
		<div className="py-10">
			<Container>
				<div className="mb-5">
					<Heading title={`Отзывы о товаре ${product.name}`} />
				</div>
				<Link
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
						...
						<div className="flex flex-col gap-2">
							...
							<select
								id="sort"
								{...register('sort')}
								className="bg-white border border-gray-300 rounded-md p-1 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
							>
				...
				{reviews.length > 0 ? (
					<div className="grid grid-cols-1 gap-10 sm:grid-cols-2 xl:grid-cols-3">
						{reviews.map(review => (
							<ReviewCart review={review} key={review.id} />
						))}
					</div>
				) : (
